...
4. 결과 처리  ==> 질의 결과를 받아서 원하는 작업을 수행한다.
	1) SQL문 select일 경우에는 select한 결과가 ResultSet객체에 저장되어
	   반환된다.
	2) SQL문이 select문이 아닐 경우(insert, update, delete문)
	   정수값이 반환된다.(정수값은 보통 실행에 성공한 레코드 수를 말한다.)   
...
*/
	public static void main(String[] args) {
		...
		try {
			...
			//     ResultSet의 데이터를 차례로 꺼내오려면
			//     반복문과 next()메서드를 이용한다.
			System.out.println("== 쿼리문 처리 결과 ==");
			
			// rs.next() ==> rs객체의 데이터를 가리키는 포인터를
			//				 다음번째 레코드로 이동시키고 그 곳에
