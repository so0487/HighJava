...
public class FileTest01 {
	...
	public static void main(String[] args) {
		...
		//File file1 = new File("c:/soo/d_other/text.txt");  // 구분문자를 '/'로 사용
		File file1 = new File("c:\\soo\\d_other\\text.txt");  // 구분문자를 '\'로 사용
		System.out.println("파일명 : " + file1.getName());
		System.out.println("디렉토리일까? : " + file1.isDirectory());
		System.out.println("파일일까? : " + file1.isFile());
		System.out.println();
		
		File file2 = new File("c:/soo/D_Other");
		...
		System.out.println("폴더명 : " + file3.getName());
		System.out.println("디렉토리일까? : " + file3.isDirectory());
		System.out.println("파일일까? : " + file3.isFile());
		System.out.println();
		
		// 3. new File(String parent, String child)
		File file4 = new File("c:/soo/d_other", "text.txt");
		System.out.println("폴더명 : " + file4.getName());
		...
		- mkdir() ==> File객체의 경로 중 마지막 위치의 디렉토리를 만든다.
				  ...
				  ==> 중간부분의 경로가 모두 만들어져 있어야
				  	  마지막 위치의 경로를 만들 수 있다.
		- mkdirs() => 중간부분의 경로가 없으면 중간부분의 경로도
				  	  같이 만들어 준다.
		*/
		File file5 = new File("c:/soo/D_Other/연습용");
		...
		File file6 = new File("c:/soo/d_other/test/java/src");
		if(!file6.exists()){
			if(file6.mkdirs()){
