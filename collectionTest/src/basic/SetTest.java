...
public class SetTest {
	...
	*/
		...
		// 삭제하는 메서드 : remove(삭제할자료) ==> 반환값 : 삭제성공(true), 실패(false)
		//                   clear() ==> 전체 삭제
		...
		*/
		...
		/*
			Set의 데이터는 순서(index)가 없기 때문에 List처럼 인덱스로 데이터를
			하나씩 불러올 수 없다.
		...
		*/
		...
		// 			==> Iterator의 포인터가 기리키는 곳 다음번째에 데이터가 있는지 검사 
		//				한다. 있으면 true,  없으면 false를 반환한다.
		while(it.hasNext()){
			// Iterator 의 next()메서드
			//  	==> Iterator의 포인터를 다음번째 위치로 이동한 후 그 곳의 데이터를
			//			반환한다.
